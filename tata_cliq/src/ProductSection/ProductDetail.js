...
import {Box, Heading, Image, Text,Button, Accordion, AccordionItem, AccordionButton, AccordionPanel, AccordionIcon, Alert, AlertIcon} from '@chakra-ui/react'
            ...
            <Box width='100%' margin={"auto"}>
                <Box  backgroundColor={'#FFFFFF'}>
                    <div className='ImageDetailDiv'>
                        <Box width={"100%"} backgroundColor={'#FFFFFF'} position={'relative'}>
                            <Image width='100%' display="block" margin="auto" src={obj.img}   />
                        ...
                        <Box backgroundColor={'#FFFFFF'} width="100%" mt={"30px"}>
                            ...
                            <Box backgroundColor={'#FFFFFF'}>
                                <Box backgroundColor={'#FFFFFF'} display={'flex'} width='100%'  margin='15px auto 5px 0px' justifyContent='space-between'>
                                    <Heading textAlign='left' backgroundColor={'#FFFFFF'} fontSize='20px' fontWeight="600" fontFamily= 'Rubik, sans-serif'  color='#181818'>{obj.brand}</Heading>
                                    <Box backgroundColor={'#FFFFFF'}>
                                        <Productwishlist data={productdata}/>
                                    </Box>
                                </Box>
                                <Box backgroundColor={'#FFFFFF'} margin='5px auto 5px auto'><Text textAlign='left'  fontSize={'22px'} fontWeight={'300'} fontFamily= 'Rubik, sans-serif' >{obj.name}</Text></Box>
                                <Box backgroundColor={'#FFFFFF'} display={'flex'} margin='20px auto 5px auto' alignItems='center'  >
                                    <Text mr={8} textAlign='center' backgroundColor={'#FFFFFF'} fontSize='24px' fontWeight={600} color='#231F20'>₹{obj.price}</Text>
                                    <Text mr={8} textAlign='center' backgroundColor={'#FFFFFF'} textDecoration={'line-through'} fontSize='17px' fontWeight={400} color='#80797B'>MRP: ₹{obj.strikedprice}</Text> 
                                    <Text mr={8} textAlign='center' backgroundColor={'#FFFFFF'} fontSize='17px' fontWeight={600} color='#499B1F'>(10% OFF)</Text>
                                </Box>
                                <Box backgroundColor={'#FFFFFF'} margin='5px auto 5px auto'>
                                    <Text backgroundColor={'#FFFFFF'} textAlign={'left'}  fontSize='15px' fontWeight={'14px'} color='#80797B'>inclusive of all taxes</Text>
                                </Box>
                                <Box backgroundColor={'#FFFFFF'} margin='25px auto 25px auto'>
                                    <Text backgroundColor={'#FFFFFF'} textAlign='left'><span style={{textAlign:'left', backgroundColor:'#49A862',color:'#FFFFFF',padding:"1px 4px"}}>4.2</span> <span style={{textAlign:'left', fontSize:'14px', fontWeight:'600' , color:'#212121'}}>30</span>  <span style={{textAlign:'left', fontSize:'14px', fontWeight:'300' , color:'#212121'}}>Ratings</span> </Text>
                                </Box>
                            </Box>
                            ...
                            {/* AVALIBLE OFFERS */}
                                <Box backgroundColor={'#FFFFFF'} padding='15px auto 5px auto'>
                                    <Text textAlign={'left'} backgroundColor={'#FFFFFF'} margin='25px auto 15px auto'color='#231F20' fontSize={'18px'} fontWeight='600' >AVAILABLE OFFERS</Text>
                                    <ProductOffer obj={offer1}/>
                                    <ProductOffer obj={offer2}/>
                                    <ProductOffer obj={offer3}/>
                                </Box>
                            {/* AVALIBLE OFFERS */}
                            <Box backgroundColor={'#FFFFFF'} margin={'20px auto'}  display={'flex'}  justifyContent='left'>
                            <Button _hover={{backgroundColor:"red"}} onClick={()=>{buyNow(productdata)}} cursor="pointer" colorScheme='red' color='white' border='none' width='195px' height='45px'margin={'2% 1%'} padding='0px 15px' fontSize='13px' fontWeight={'600'} backgroundColor={'rgb(218, 28, 92)'}   borderRadius='22px' variant='outline'>
                                BUY NOW
                            </Button>
                            <Button disabled={disabled?true:false}backgroundColor={disabled?"grey":'#FFFFFF'}  onClick={()=>{addNow(productdata)}} cursor={disabled?"not-allowed":'pointer'} color={disabled?"white":'#FF1744'} border={disabled?"2px solid grey":'2px solid #FF1744'} margin={'2% 1%'} width='195px' height='45px' padding='0px 15px' fontSize='13px' fontWeight={'600'}    borderRadius='22px' variant='outline'>
                                ADD TO BAG
                            </Button>
